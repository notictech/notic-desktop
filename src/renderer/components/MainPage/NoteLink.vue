<template>
    <div :id="'notelink_index_' + index">
        <b-button size="sm" :class="[{active: activeNoteIndex == index}, 'note-link', 'btn', 'btn-outline-primary', 'btn-sm']" @click="goToNote(index)" @mousedown.middle="openEditNotePage(note._id)"><icon class="is-star" v-show="note.star && !note.deleted" name="star"></icon><icon class="is-secret" v-show="note.secrets && !note.deleted" name="key"></icon><icon class="is-reminder" v-show="note.reminder && !note.deleted" name="bell"></icon> {{ note.title }}</b-button>
    </div>
</template>
